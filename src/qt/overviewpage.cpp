...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeLithiumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLithiumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeLithiumAmount = strAnonymizeLithiumAmount.remove(strAnonymizeLithiumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLithiumAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeLithiumAmount * COIN) nMaxToAnonymize = nAnonymizeLithiumAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeLithiumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeLithiumAmount));
        strAnonymizeLithiumAmount = strAnonymizeLithiumAmount.remove(strAnonymizeLithiumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLithiumAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
