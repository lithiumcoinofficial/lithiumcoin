...
Lithium now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<lithium_address\> \<monthly_payment_lithium\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ LIT, _12_ times over the course of a year totaling _24000_ LIT.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<lithium_address\> \<monthly_payment_lithium\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` LIT.
 ...
 * vote-many          - Vote on a Lithium initiative
 * vote-alias         - Vote on a Lithium initiative
 * vote               - Vote on a Lithium initiative/budget
