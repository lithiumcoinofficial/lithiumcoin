...
The LIT Core project operates an open contributor model where anyone is
...
The following applies to code changes to the LIT Core project, and is not to be
confused with overall LIT Network Protocol consensus changes.

Whether a pull request is merged into LIT Core rests with the project merge
...
Patches that change LIT consensus rules are considerably more involved than
...
Where a patch set proposes to change the LIT consensus, it must have been
...
The project leader is the release manager for each LIT Core release.
